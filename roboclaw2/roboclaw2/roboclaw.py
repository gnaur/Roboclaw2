...
def _sendcommand(address,command):
	crc_clear()
	crc_update(address)
	port.write(bytes([address&0xFF]))
	crc_update(command)
	port.write(bytes([command&0xFF]))
	return
...
def SetM1PositionPID(address,kp,ki,kd,kimax,deadzone,min,max):
	return _write4444444(address,Cmd.SETM1POSPID,(kd*1024),(kp*1024),(ki*1024),kimax,deadzone,min,max)

def SetM2PositionPID(address,kp,ki,kd,kimax,deadzone,min,max):
	return _write4444444(address,Cmd.SETM2POSPID,(kd*1024),(kp*1024),(ki*1024),kimax,deadzone,min,max)
